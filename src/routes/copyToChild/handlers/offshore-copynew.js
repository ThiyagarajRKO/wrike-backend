...
export const OffshoreCopynew = (params, startedAt, fastify) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      resolve({
        message:
          "CopyToChild - Offshore copynew process has been created successfully",
        data: {},
      });
    } catch (err) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const taskOutput = await GetResponse(
        `${WrikeEndpoint}/folders/${folderId}/tasks?subTasks=true&pageSize=20&fields=[customFields]&nextPageToken=${taskTempToken ?? ""}`,
        "GET",
        {
          "content-type": "application/json",
          Authorization: `Bearer ${WrikeToken}`,
